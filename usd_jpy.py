...
def trade_usdjpy():
    ...
    usdjpy = instruments.InstrumentsCandles(instrument="USD_JPY", params = params)
    usdjpy_candles = client.request(usdjpy)


    ## Puts candle close time into a list
    candle_times = []
    candle_close_time = usdjpy_candles['candles']
    ...
    df5 = pd.DataFrame(zippedList, columns = ['open' , 'high', 'low', 'close'])
    ...
    df5['Time'] = candle_times
    df5['USD/JPY'] = df5['Time'].map(lambda x: str(x)[:-11])
