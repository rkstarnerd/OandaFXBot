...
def trade_eurgbp():
    ...
    eurgbp = instruments.InstrumentsCandles(instrument="EUR_GBP", params = params)
    eurgbp_candles = client.request(eurgbp)


    ## Puts candle close time into a list
    candle_times = []
    candle_close_time = eurgbp_candles['candles']
    ...
    df4 = pd.DataFrame(zippedList, columns = ['open' , 'high', 'low', 'close'])
    ...
    df4['Time'] = candle_times
    df4['EUR/GBP'] = df4['Time'].map(lambda x: str(x)[:-11])
