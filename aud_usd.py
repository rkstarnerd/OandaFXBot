...
def trade_audusd():
    ...
    audusd = instruments.InstrumentsCandles(instrument="AUD_USD", params = params)
    audusd_candles = client.request(audusd)


    ## Puts candle close time into a list
    candle_times = []
    candle_close_time = audusd_candles['candles']
    ...
    df3 = pd.DataFrame(zippedList, columns = ['open' , 'high', 'low', 'close'])
    ...
    df3['Time'] = candle_times
    df3['AUD/USD'] = df3['Time'].map(lambda x: str(x)[:-11])
