...
def trade_gbpusd():
    ...
    gbpusd = instruments.InstrumentsCandles(instrument="GBP_USD", params = params)
    gbpusd_candles = client.request(gbpusd)


    ## Puts candle close time into a list
    candle_times = []
    candle_close_time = gbpusd_candles['candles']
    ...
    df2 = pd.DataFrame(zippedList, columns = ['open' , 'high', 'low', 'close'])
    ...
    df2['Time'] = candle_times
    df2['GBP/USD'] = df2['Time'].map(lambda x: str(x)[:-11])
